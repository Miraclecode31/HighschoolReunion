...
const CommentBox = () => {
    ...
    useEffect(() => {
        const fetchComments = async () => {
            try {
                const response = await fetch('http://localhost:3000/api/schools/67f6fee746e000a802b3e3dc/comments'); 
                if (response.ok) {
    ...
    return (
        <div className="absara-comment fixed bottom-20 left-0 right-0 px-4">
            <AnimatePresence>
                {showComments && (
                    <motion.div
                    ...
                    >
                        <div className="absara-comment-scroll max-h-40 overflow-y-auto">
